...
export class HighlightQueryPipe implements PipeTransform {
  ...
  //     console.log('selectedKeywordsList',words[i])
  ...
  // //      if (Math.abs(pos1 - pos2) <= vicinity || Math.abs(pos2 - pos1) <= vicinity) {
  ...
  // //   console.log('regexVicinity',pairs)
  ...
  transform(content: string, matchSentences: string[], keywords: string[], matchedkeyword: any): string {
    let highlightedContent = content;

    // matchedkeyword.forEach((each: any) => {
    //   console.log(each)
    // })
    ...
    matchSentences.forEach((sentence) => {
      ...
      const regex2 = new RegExp(`(${sentence})`, 'gi');
      highlightedContent = highlightedContent.replace(regex2, '<span class="highlight-vicinity">$&</span>');
    });


    keywords.forEach((keyword) => {
      if (keyword) {
        const regex3 = new RegExp(`(${keyword})`, 'gi');
        highlightedContent = highlightedContent.replace(regex3, `<span class="highlight-keyword">$&</span>`);
      }
    });
    for (const [key, value] of Object.entries(matchedkeyword)) {
      if (matchedkeyword[key] > 0) {
        // console.log(`Key: ${key}, Value: ${value}`);
        const regex1 = new RegExp(`(${key})`, 'gi');
        highlightedContent = highlightedContent.replace(regex1, '<span class="highlight-word2">$&</span>');
      }
    }
